...
from myst_parser.inventory import filter_inventories, from_sphinx
from myst_parser.mdit_to_docutils.local_links import (
    LocalAnchorType,
    LocalInvMatch,
    MystInventoryType,
    filter_myst_inventory,
    gather_anchors,
    ...
    def _resolve_xref_inventory(
        ...
        results = list(
            filter_inventories(
                ...
                ("*" if match_end else "") + ref_target,
                ref_inv=ref_inv,
                ref_domain=ref_domain,
                ref_otype=ref_object_type,
                fnmatch_target=match_end,  # TODO maybe change this in the syntax
            )
        )
        loc = ":".join(
            [
