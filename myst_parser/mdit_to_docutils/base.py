...
class DocutilsRenderer(RendererProtocol):
    ...
    def run_directive(
        ...
        if (
            first_line
            and self.md_config.strict_arguments
            and issubclass(directive_class, BaseAdmonition)
            and not directive_class.required_arguments
            and not directive_class.optional_arguments
            and hasattr(directive_class, "node_class")
        ):
            # the first line is very likely intended as the admonition title
            # so, instead of failing, we instead convert it to an admonition
            # with a class of the directive name
            insert_class = directive_class.node_class.__name__
            directive_class = Admonition
        ...
        try:
            parsed = parse_directive_text(
                ...
                first_line,
                content,
                strict_argument=self.md_config.strict_arguments,
            )
        ...
        if insert_class:
            parsed.options.setdefault("class", []).insert(0, insert_class)
